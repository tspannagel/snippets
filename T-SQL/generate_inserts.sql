...
AS(
    SELECT TOP(100000000)
        ...
        , 'ISNULL('  + CASE WHEN ty.name IN ('varchar', 'nvarchar', 'datetime', 'datetime2', 'datetimeoffset') 
                            THEN 'CASE WHEN ' + c.name + ' IS NULL THEN NULL ELSE '''''''' END +' 
                            ELSE '' 
                        END
                    + CASE WHEN ty.name IN ('int','bigint', 'float', 'decimal','datetime', 'datetime2', 'datetimeoffset','bit') 
                            THEN ' CAST('
                            ELSE ''
                     ...
                     + CASE WHEN ty.name IN ('int','bigint', 'float', 'decimal','datetime', 'datetime2', 'datetimeoffset','bit') 
                            ...
                            ELSE '' 
                        END
                    + CASE WHEN ty.name IN ('varchar', 'nvarchar', 'datetime', 'datetime2', 'datetimeoffset') 
                            THEN '+ CASE WHEN ' + c.name + ' IS NULL THEN NULL ELSE '''''''' END' 
                            ELSE ''
                         END
